...
在开始之前，我们需要准备一些Python工具包，它们将帮助我们处理数据、进行计算和可视化结果。以下是你需要的内容：
...
假设你是一名大学管理员，你希望根据申请者的两次考试成绩预测他们是否会被录取。你有历史数据：考试成绩和录取结果（1表示录取，0表示未录取）。我们的目标是创建一个模型，估计录取的概率。
...
逻辑回归使用Sigmoid函数将预测值映射到0到1之间：

![Sigmoid 函数](https://latex.codecogs.com/png.latex?g(z)%20=%20\frac{1}{1%20+%20e^{-z}})
...
Sigmoid函数将任何实数映射到(0, 1)——非常适合表示概率！
...
![成本函数](https://latex.codecogs.com/png.latex?J(w,%20b)%20=%20\frac{1}{m}%20\sum_{i=0}^{m-1}%20\left[-y^{(i)}\log(f(x^{(i)}))%20-%20(1%20-%20y^{(i)})\log(1%20-%20f(x^{(i)}))\right])
...
![梯度 ∂J/∂b](https://latex.codecogs.com/png.latex?\frac{\partial%20J}{\partial%20b}%20=%20\frac{1}{m}%20\sum_{i=0}^{m-1}%20(y^{(i)}%20-%20a^{(i)}))

![梯度 ∂J/∂w_j](https://latex.codecogs.com/png.latex?\frac{\partial%20J}{\partial%20w_j}%20=%20\frac{1}{m}%20\sum_{i=0}^{m-1}%20(f(x^{(i)})%20-%20y^{(i)})%20x_j^{(i)})
...
假设你是一名工厂经理，通过两次测试来检测芯片。你希望根据测试成绩预测芯片是否通过质量检测（1表示通过，0表示未通过）。
...
从2个特征扩展到27个特征——这将有助于拟合数据，但也可能导致过拟合。
...
![正则化成本函数](https://latex.codecogs.com/png.latex?J(w,%20b)%20=%20\frac{1}{m}%20\sum_{i=0}^{m-1}%20\left[-y^{(i)}\log(f(x^{(i)}))%20-%20(1%20-%20y^{(i)})\log(1%20-%20f(x^{(i)}))\right]%20+%20\frac{\lambda}{2m}%20\sum_{j=0}^{n-1}%20w_j^2)
...
![正则化梯度](https://latex.codecogs.com/png.latex?\frac{\partial%20J}{\partial%20w_j}%20=%20\frac{1}{m}%20\sum_{i=0}^{m-1}%20(f(x^{(i)})%20-%20y^{(i)})%20x_j^{(i)}%20+%20\frac{\lambda}{m}%20w_j)
...
w, b, J_history = gradient_descent(X_mapped, y_train, initial_w, initial_b, 
                                   compute_cost_reg, compute_gradient_reg, 0.01, 10000, lambda_)
```
