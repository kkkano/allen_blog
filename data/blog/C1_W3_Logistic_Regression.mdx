...
J(w, b) = (1/m) * sum_{i=0}^{m-1} [ -y^(i) * log(f(x^(i))) - (1 - y^(i)) * log(1 - f(x^(i))) ]
...
∂J/∂b = (1/m) * sum_{i=0}^{m-1} (y^(i) - a^(i))

∂J/∂w_j = (1/m) * sum_{i=0}^{m-1} (f(x^(i)) - y^(i)) * x_j^(i)
...
J(w, b) = [原始成本] + (lambda / (2m)) * sum_{j=0}^{n-1} w_j^2
