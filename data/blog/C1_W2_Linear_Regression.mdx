...
**说明：**  
这个 `utils.py` 文件模拟了一个包含 10 个样本的数据集：
...
**梯度下降法**通过计算成本函数的梯度，逐步调整 `w` 和 `b`，减小误差。步骤：
1. 计算成本对 `w` 和 `b` 的偏导数（梯度）。
2. 用学习率 `alpha` 更新参数。
3. 重复直到成本收敛。
...
恭喜你完成这场线性回归实践！通过这篇博客，你学会了：
- 用线性回归拟合数据，找到人口与利润的关系。
- 实现成本函数和梯度下降，优化模型参数。
- 用图表验证模型效果并预测新数据。
- 这只是机器学习的起点！试着调整 `alpha` 或迭代次数，看看效果如何变化。下一期我们将探索分类问题，敬请期待！
本篇文章的部分内容和思想参考了 [吴恩达 (Andrew Ng)](https://www.andrewng.org/) 在 [Coursera 机器学习课程](https://www.coursera.org/learn/machine-learning) 中的讲解，感谢其对机器学习领域的卓越贡献。