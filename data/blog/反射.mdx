...
1. **获取Class对象**：通过 `Class.forName()`、`对象.getClass()` 或 `类名.class` 三种方式获取目标类的 `Class` 对象。
...
- **异常处理不可忽视**：反射操作涉及 `ClassNotFoundException`、`NoSuchMethodException`、`IllegalAccessException` 等多种受检异常，务必妥善处理。
