...
![让 Bot 开浏览器 → 找不到 Chrome，同时 web_search 也没配好](/static/images/openclaw-deployment/no-browser-no-search.jpg)
...
![限流重试配置生效后的日志](/static/images/openclaw-deployment/140e928b1822ed5b3fb7fe10a650a0f0.jpg)
...
后来才想明白：DeepWiki 里很多字段是**内部对象属性**，并不一定暴露成 `openclaw.json` 可写配置。
...
![Bot 坚持说自己在 claude.ai](/static/images/openclaw-deployment/e0b75eafdcabfc0dac2eb9193daa3e9d.jpg)
...
![三层环境注入配置](/static/images/openclaw-deployment/9309c27f2f49b38de4096d3fa3e5567f.jpg)
...
![subagent 配对权限修复](/static/images/openclaw-deployment/a6228456d4390c12b6e00f99a243e88e.jpg)
...
![thinking 档位与模型链路配置](/static/images/openclaw-deployment/aad330b56b113e5e353a5e17b5cbfaec.jpg)
...
回看文件才发现：有一段字符串被写进了**真实换行符**——不是 `\n`，而是文件里真的换行了，直接把 JSON 字符串结构破坏掉。

怀疑根因是 OpenClaw 某处 session store 的写入逻辑（`saveSessionStoreUnlocked` 这类路径）在某些情况下把内容"原样落盘"，导致配置被污染。

![openclaw.json 被写坏的现场](/static/images/openclaw-deployment/c306714ae1c5b553beb672d7e6f285f0.jpg)

解决办法很朴素，但很痛：

1. **不要在坏掉的 JSON 上修修补补**（越修越乱）
2. 直接拿一份干净的结构**重建 openclaw.json**
3. 把大段 prompt 文本统一改成：
   - 单行 JSON 字符串（用 `\n` 转义换行）
   - 或者放到独立文件（SOUL.md）里引用

![重建配置后 gateway 恢复正常](/static/images/openclaw-deployment/e1ffc4b2f5dcfd8636628cdf2bbe25ca.jpg)
