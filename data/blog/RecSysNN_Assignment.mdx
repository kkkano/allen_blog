...
数学上，成本函数为：
$$ J = \frac{1}{2} \sum\_{(i,j): r(i,j)=1} (\mathbf{w}^{(j)} \cdot \mathbf{x}^{(i)} + b^{(j)} - y^{(i,j)})^2 + \frac{\lambda}{2} \left( \sum_j \sum_k (\mathbf{w}\_k^{(j)})^2 + \sum_i \sum_k (\mathbf{x}\_k^{(i)})^2 \right) $$
其中 $ \lambda $ 是正则化参数。
...
<IframeEmbed
  src="/visualizations/recsysnn-embedding-space.html"
  minHeight={760}
  title="RecSysNN 嵌入空间可视化"
/>

<IframeEmbed
  src="/visualizations/recsysnn-training-lab.html"
  minHeight={760}
  title="RecSysNN 训练动态实验"
/>

<IframeEmbed
  src="/visualizations/recsysnn-ranking-simulator.html"
  minHeight={760}
  title="RecSysNN 排序模拟器"
/>
